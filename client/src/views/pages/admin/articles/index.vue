      ...
      <div class="row">
        <div v-bind:class="[{'col-md-5': selected !== null}, {'col-md-12': selected === null}]" class="col-md-5">
          ...
          <table class="table table-condensed table-hover">
            ...
            <tbody>
            <tr v-on:click="select(item)" v-for="item in articles" v-bind:key="item._id">
              <td>{{item.date}}</td>
              <td>{{item.title}}</td>
              <td>
...
export default {
  ...
  methods: {
    ...mapActions('articles', {
      get: action.GET_ARTICLES,
      _post: action.POST_ARTICLES,
      _remove: action.DELETE_ARTICLES,
      put: action.PUT_ARTICLES
    }),
    async post (data) {
