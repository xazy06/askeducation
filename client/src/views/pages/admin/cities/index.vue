      ...
      <div class="row">
        <div v-bind:class="[{'col-md-6': selected !== null}, {'col-md-12': selected === null}]" class="col-md-6">
          ...
          <table class="table table-condensed table-hover">
            ...
            <tbody>
            <tr v-on:click="select(item)" v-for="item in cities" v-bind:key="item._id">
              <td>{{item.name}}</td>
              <td>
...
export default {
  ...
  methods: {
    ...mapActions('cities', {
      get: action.GET_CITIES,
      _post: action.POST_CITIES,
      _remove: action.DELETE_CITIES,
      put: action.PUT_CITIES
    }),
    async post (data) {
